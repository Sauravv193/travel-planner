  ...
  return (
    <div className={`transform transition-all duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
      <div className="space-y-8">
        {/* Enhanced Header */}
        <div className="bg-gradient-to-br from-pink-500 via-pink-600 to-pink-400 rounded-2xl shadow-2xl overflow-hidden relative">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-pink-100 text-lg md:text-xl leading-relaxed max-w-2xl">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {processedItinerary.tips.map((tip, index) => (
                <div key={index} className="flex items-start bg-white p-4 rounded-xl shadow-sm">
                  ...
                  <span className="text-orange-700 font-medium">{tip}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="relative">
          {/* Main Timeline Line */}
          <div className="absolute left-8 top-0 bottom-0 w-1 bg-gradient-to-b from-pink-500 via-pink-600 to-pink-400 rounded-full shadow-lg"></div>
              ...
              >
                ...
                <div className="flex items-start relative">
                  ...
                  <div className="flex-shrink-0 mr-8 relative z-10">
                    <div className="w-16 h-16 bg-gradient-to-br from-pink-500 to-pink-600 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg border-4 border-white">
                      {index + 1}
                    </div>
                    {index < processedItinerary.days.length - 1 && (
                      <div className="absolute top-16 left-1/2 transform -translate-x-1/2 w-px h-12 bg-gradient-to-b from-pink-400 to-transparent"></div>
                    )}
                  </div>
        ...
        {processedItinerary.essentialTips && processedItinerary.essentialTips.length > 0 && (
          <div className="bg-gradient-to-br from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border border-purple-200 dark:border-purple-800/30 rounded-2xl p-8 shadow-lg">
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-purple-200 dark:bg-purple-800/30 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg className="w-8 h-8 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <h3 className="text-2xl font-bold text-purple-800 dark:text-purple-300 mb-2">✨ Essential Travel Tips</h3>
              <p className="text-purple-600 dark:text-purple-400">Important insights for a smoother journey</p>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {processedItinerary.essentialTips.map((tip, index) => (
                <div key={index} className="bg-white dark:bg-night-surface p-4 sm:p-5 rounded-xl shadow-sm border border-purple-100 dark:border-purple-800/20 hover:shadow-md transition-shadow duration-200">
                  <div className="flex items-start">
                    <div className="w-6 h-6 sm:w-8 sm:h-8 bg-purple-100 dark:bg-purple-800/30 rounded-full flex items-center justify-center mr-3 sm:mr-4 flex-shrink-0 mt-1">
                      <span className="text-purple-600 dark:text-purple-400 font-bold text-xs sm:text-sm">{index + 1}</span>
                    </div>
                    <div className="flex-1">
                      <p className="text-purple-800 dark:text-purple-200 font-medium text-sm sm:text-base leading-relaxed">{tip}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="card p-8 text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-pink-500 to-pink-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg shadow-pink-500/20">
            <svg className="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
            </svg>
          </div>
          <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Your Journey Awaits!</h3>
          <p className="text-gray-600 dark:text-night-muted max-w-2xl mx-auto">
