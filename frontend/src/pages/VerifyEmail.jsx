...
const VerifyEmail = () => {
  ...
  if (status === 'loading') {
    return <div className="min-h-screen flex items-center justify-center"><LoadingSpinner size="large" text="Verifying your email..." /></div>;
  }
      ...
      <div className="flex items-center justify-center min-h-screen px-4">
        <div className="auth-glass-card text-center">
          ...
          <h1 className={`text-2xl font-bold mb-2 ${
            status === 'success' ? 'text-warm-dark dark:text-white' : 'text-red-800 dark:text-red-400'
          }`}>
            {status === 'success' ? 'Email Verified!' : 'Verification Failed'}
          </h1>
          <p className="text-brown-600 dark:text-night-muted mb-6">{message}</p>
          <Link to="/signin" className="btn-primary py-2.5 px-6 inline-block">Go to Sign In</Link>
        </div>
      </div>
    </div>
  );
